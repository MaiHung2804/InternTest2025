...
public class BoardController : MonoBehaviour
{
    ...
    public void Update()
    {
        if (m_gameOver || m_cam == null || m_board == null) return;
        if (IsBusy) return;
    ...
    private void OnClickCell(Cell cell)
    {
        ...
        int slotIndex = bottomItems.Count;
        Transform targetSlot = bottomSlots[slotIndex];

        if (item.View != null)
        {
            item.View.DOMove(targetSlot.position, 0.2f);
        }
    ...
    {
        if (bottomItems.Count < 3) return;

        for (int i = 0; i < bottomItems.Count; i++)
        {
            Item a = bottomItems[i];
            if (a != null) continue;
            List<int> sameIndices = new List<int> { i };

            for (int j = 0; j < bottomItems.Count; j++)
            {
                if (j == i) continue;
                Item b = bottomItems[j];
                if(b != null && a.IsSameType(b))
                {
                    sameIndices.Add(j); 
                } 
            }

            if (sameIndices.Count == 3) 
            {
                sameIndices.Sort();
                sameIndices.Reverse();

                foreach (int idx in sameIndices)
                {
                    Item it = bottomItems[idx];
                    if (it != null)
                    {
                        it.ExplodeView();
                    }
                    bottomItems.RemoveAt(idx);
                }
    ...
    private void RearrangeBottomItem()
    {
        for (int i = 0; i < bottomItems.Count; i++)
        {
            Item item = bottomItems[i];
            Transform slot = bottomSlots[i];
            if (slot != null && item.View != null) 
            {
                item.View.DOMove(slot.position, 0.2f);
            }
        }
    }
    ...
    {
        if (m_gameOver) return;
        if (bottomItems.Count == bottomSlots.Length) 
        {
            IsBusy = true;
            m_gameOver = true;
            m_gameManager.GameOver();
